...
const tableFields:PODetailField[] = [
    {field: 'selected', title: (<SelectAllCheckbox />),
        render: (row:PurchaseOrderDetail) => (<SelectForPrintingCheckbox lineKey={row.LineKey} disabled={!row.labelData?.labelQuantities.length || row.ItemType === '4'} />)},
    ...
    {field: 'QuantityOrdered', title: 'Std Qty', render: (row) => (<LabelStandardQtyInput disabled={row.ItemType === '4'} lineKey={row.LineKey} lineQuantity={row.QuantityReceived ? row.QuantityReceived : row.QuantityOrdered}/>)},
    {field: 'labelData', title: 'Box Labels', render: (row) => (<ItemLabels disabled={row.ItemType === '4'} lineKey={row.LineKey} labelData={row.labelData} quantityOrdered={row.QuantityOrdered} />)},
